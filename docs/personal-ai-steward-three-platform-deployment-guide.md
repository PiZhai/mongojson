...
脚本会先构建前端、运行 `go test ./cmd/steward`，然后交叉编译：
...
该命令会核对五个 target、manifest、所有文件的 SHA-256 及当前平台二进制版本。`-RunCurrentBinary` 只会运行当前构建机平台的二进制，其他平台需在对应目标机再次执行 `steward version`。
...
该命令会真实注册、启动并严格验证服务，默认执行 advisor 可用性、D2 隐私阻断和 24 小时 watch。首次调试可把 `WatchDuration` 临时改为 `10m`，但不能用短验证声称最终验收完成。
...
成功条件包括：管理面仅 loopback、Peer 安全材料完整、数据库可用、低风险写入探针成功、D0 advisor 调用成功、D2 数据在发送到模型前被阻断。
...
| privacy probe 失败 | D2 数据被实际发送，或策略未生效 | 立即停止 advisor，保留 evidence 并排查隐私门禁 |
...
- [ ] 工作台可从每台设备本机打开。
- [ ] 指定模型的 D0 probe 成功，D2 privacy probe 被本地阻断。
