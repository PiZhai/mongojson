...
音乐模块通过 `POST /api/music/tracks` 持久化上传歌曲，通过 `GET /api/music/tracks?cursor=...&limit=...` 游标分页读取远程曲库，并通过 `GET /api/music/tracks/{id}/content` 获取支持 HTTP Range 的音频内容。音乐文件使用独立 `music` 存储分类且不进入临时文件过期清理。
