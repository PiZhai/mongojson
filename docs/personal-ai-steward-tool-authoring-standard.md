# Steward Tool Authoring Standard 1.1
...
所有会改变操作系统状态的生成工具都应声明 `transaction.mode: automatic`，并提供三个同包、同版本入口：

- `snapshot_entrypoint`：操作前读取最小可恢复状态。
- `verification_entrypoint`：从操作系统重新读取状态，只有返回 `{"verified":true}` 才能提交。
- `rollback_entrypoint`：仅撤销本次调用拥有的变化，成功时返回 `{"rolled_back":true}`。

三个入口都使用 `steward-tool/1`。运行时在工具执行前持久化快照，在验证后提交；执行、验证、取消或进程崩溃时执行补偿。回滚不得用旧快照覆盖其他进程随后写入的无关变化，应优先做差异协调。例如 PATH 回滚只移除本事务增加的目录。

## `steward-tool/1` 精确协议

脚本从 stdin 读取一行 NDJSON 请求：

```json
{"protocol":"steward-tool/1","invocation_id":"...","arguments":{},"context":{"device_id":"...","known_folders":{},"working_directory":"..."}}
```

最后一个非空 stdout 行必须是以下两种响应之一，不能直接输出业务对象：

```json
{"ok":true,"output":{"value":"business result"},"evidence":[]}
{"ok":false,"error":"actionable failure message","evidence":[]}
```

Manifest 的 `input_schema` 校验 `arguments` 内部的业务参数，`output_schema` 校验 `output` 内部的业务结果。`tests[].input` 直接填写业务参数，例如 `{"scope":"User"}`；运行时会自动创建协议外壳，禁止再写成 `{"arguments":{"scope":"User"}}`。

PowerShell 入口必须避免使用自动变量 `$args` 保存业务参数，推荐从以下模板开始：
...
try {
    ...
    $toolArguments = if ($null -eq $request.arguments) { [pscustomobject]@{} } else { $request.arguments }
    $output = [ordered]@{ value = 'business result' }
    $response = [ordered]@{ ok = $true; output = $output; evidence = @() }
} catch {
    $response = [ordered]@{ ok = $false; error = $_.Exception.Message; evidence = @() }
}
[Console]::Out.WriteLine(($response | ConvertTo-Json -Depth 20 -Compress))
```

Python 与 Node.js 使用同样的外壳。Python 最后一行使用 `json.dumps({"ok": True, "output": output, "evidence": []})`；Node.js 使用 `JSON.stringify({ok: true, output, evidence: []})`。诊断日志可以先输出，但最终非空行必须是响应外壳。
...
- 未先搜索目录就创建与现有工具重复的脚本。
- 改变系统状态却只依赖脚本输出“成功”，没有重新读取后置状态。
- 回滚时用完整旧值覆盖其他进程产生的并发修改。
