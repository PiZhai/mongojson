...
func (s *Service) ensureAutomationPolicyDefaults(ctx context.Context, now time.Time) error {
	dataDefaults := []domain.StewardDataPolicy{
		...
		{DataLevel: DataD2, SourcePattern: "proactive:*", CollectMode: PolicyModeAuto, ModelMode: PolicyModeAuto, ModelContentMode: ModelContentRedacted, AllowLocalPersistence: true, AllowSync: false, Description: "允许主动管家发送本地聚合且已脱敏的活动摘要"},
