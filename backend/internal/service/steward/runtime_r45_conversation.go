...
func (s *Service) createConversationExecutionFromPlan(ctx context.Context, conversation domain.StewardConversation, userMessage domain.StewardConversationMessage, instruction, level string, target conversationExecutionTarget, plan RuntimePlanDraft) (domain.StewardConversationMessage, domain.StewardConversationExecution, error) {
	...
	if !target.Remote && len(plan.Steps) == 1 {
		run, createErr := s.CreateAgentRun(ctx, CreateAgentRunInput{
			Goal: summary, Mode: "planned", IdempotencyKey: "conversation:" + userMessage.ID,
		...
		orchestration, createErr := s.createConversationOrchestration(ctx, userMessage.ID, summary, level, permission, target, plan.Steps)
		if createErr != nil {
			return assistant, execution, createErr
		}
...
func (s *Service) createConversationOrchestration(ctx context.Context, requestMessageID, summary, level, permission string, target conversationExecutionTarget, steps []CreateAgentRunStepInput) (domain.StewardOrchestration, error) {
	...
	for _, step := range steps {
		...
		nodes = append(nodes, CreateOrchestrationNodeInput{
			...
			TargetDevice: targetDevice, DependsOn: append([]string(nil), step.DependsOn...),
	...
	return s.CreateOrchestration(ctx, CreateOrchestrationInput{
		Goal: summary, IdempotencyKey: "conversation:" + requestMessageID, RequestedBy: "conversation",
