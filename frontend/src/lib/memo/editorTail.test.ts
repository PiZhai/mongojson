...
import { ensureTrailingNewlines, getTailClickLine } from './editorTail'

describe('memo editor tail helpers', () => {
  ...
  it('pads to the requested double-click tail line', () => {
    expect(ensureTrailingNewlines('done', 2)).toBe('done\n\n')
    expect(ensureTrailingNewlines('done\n', 3)).toBe('done\n\n\n')
  })

  it('keeps an empty document empty on first-line single click', () => {
    expect(ensureTrailingNewlines('', 1)).toBe('')
  })

  it('pads an empty document to the requested blank line for double click', () => {
    expect(ensureTrailingNewlines('', 3)).toBe('\n\n')
  })

  it('returns the tail line based on the click distance after the last content block', () => {
    expect(getTailClickLine(101, 100, 20)).toBe(1)
    expect(getTailClickLine(125, 100, 20)).toBe(2)
    expect(getTailClickLine(200, 100, 20)).toBe(3)
    expect(getTailClickLine(100, 100, 20)).toBeNull()
  })
})
