...
const insertCodeWbr = (codeElement: HTMLElement) => {
    ...
    if (codeText === "" || codeText === "\n") {
        codeElement.innerHTML = `${Constants.ZWSP}<wbr>\n`;
        return;
    }
...
const markInsertedIRCodeBlock = (pasteElement: HTMLElement, vditor: IVditor) => {
    if (vditor.currentMode !== "ir") {
        return false;
    }
    const codeBlockElement = pasteElement.lastElementChild as HTMLElement;
...
export const insertHTML = (html: string, vditor: IVditor) => {
    ...
    if (pasteElement.firstElementChild &&
        ...
        // 粘贴内容为块元素时，应在下一段落中插入
        if (!markInsertedIRCodeBlock(pasteElement, vditor)) {
