import DiffMatchPatchConstructor, * as DiffMatchPatch from "diff-match-patch";
...
interface IDiffMatchPatch {
    diff_main(text1: string, text2: string, optChecklines?: boolean): DiffMatchPatch.Diff[];
    patch_apply(patches: DiffMatchPatch.patch_obj[], text: string): [string, boolean[]];
    patch_deepCopy(patches: DiffMatchPatch.patch_obj[]): DiffMatchPatch.patch_obj[];
    patch_make(text1: string, text2: string, diffs: DiffMatchPatch.Diff[]): DiffMatchPatch.patch_obj[];
}

class Undo {
    private stackSize = 50;
    private dmp: IDiffMatchPatch;
    ...
    constructor() {
        this.resetStack();
        this.dmp = new DiffMatchPatchConstructor();
    }
