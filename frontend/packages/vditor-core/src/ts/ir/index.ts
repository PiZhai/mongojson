...
const removeFocusedCodeBlockCaretPadding = () => {
    ...
    codeElement.childNodes.forEach((node) => {
        if (node.nodeType !== 3 || !node.nodeValue.includes(Constants.ZWSP)) {
            return;
        }

        const startOffset = node.isSameNode(range.startContainer) ? range.startOffset : null;
        const removedBeforeStart = startOffset === null
            ? 0
            : node.nodeValue.slice(0, startOffset).split(Constants.ZWSP).length - 1;
        node.nodeValue = node.nodeValue.replace(new RegExp(Constants.ZWSP, "g"), "");

        if (startOffset !== null) {
            range.setStart(node, Math.max(0, startOffset - removedBeforeStart));
            range.collapse(true);
 ...
 contenteditable="true" spellcheck="false"></pre>`;
    ...
    private bindEvent(vditor: IVditor) {
        ...
        this.element.addEventListener("beforeinput", () => {
            removeFocusedCodeBlockCaretPadding();
        });
