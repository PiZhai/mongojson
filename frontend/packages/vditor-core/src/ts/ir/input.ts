...
const shouldKeepCodeBlockExpanded = (blockElement: false | HTMLElement, range: Range) => {
    return Boolean(blockElement &&
        blockElement.getAttribute("data-type") === "code-block" &&
        hasClosestByClassName(range.startContainer, "vditor-ir__marker--pre"));
};

const expandFocusedCodeBlock = () => {
...
const removeCodeBlockCaretPadding = (blockElement: HTMLElement) => {
    if (blockElement.getAttribute("data-type") !== "code-block") {
        return;
    }
    const codeElement = blockElement.querySelector<HTMLElement>(".vditor-ir__marker--pre code");
    if (!codeElement) {
        return;
    }
...
export const input = (vditor: IVditor, range: Range, ignoreSpace = false, event?: InputEvent) => {
    ...
    if (!isIRElement) {
        ...
        removeCodeBlockCaretPadding(blockElement);
    ...
    if (keepCodeBlockExpanded) {
        expandFocusedCodeBlock();
    }
